...
class SingleLayerTexture(nn.Module):
    def __init__(self, W, H):
        ...
        self.layer1 = nn.Parameter(torch.FloatTensor(1, 1, W, H)).to(device=DEVICE)
...
class NeuralTexture(nn.Module):
    def __init__(self, W, H, feature_num, use_pyramid=True):
        ...
        if self.use_pyramid:
            self.textures = nn.ModuleList([LaplacianPyramid(W, H) for i in range(feature_num)]).to(device=DEVICE)
